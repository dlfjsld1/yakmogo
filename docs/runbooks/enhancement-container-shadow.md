...
Pi 전체를 재설치하거나 Docker 서비스를 함께 복구해야 한다면 [Pi Docker 운영 복구 런북](pi-docker-recovery.md)을 먼저 읽는다. 이 shadow 런북은 Yakmogo 한 서비스의 검증 절차만 다룬다.

## 검증된 현재 값

```text
shadow URL: http://192.168.50.192:18081
