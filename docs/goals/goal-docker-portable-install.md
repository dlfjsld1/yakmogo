...
**완료.** 2026-07-15 GitHub-hosted ARM64 검증과 Pi 실기 검증에서 신규 설치, Flyway bootstrap, SQL 백업, 새 MariaDB volume 복원, 18081 shadow, 8081 전환과 고장 release 자동 rollback이 성공했다. 운영 8080, 운영 DB `yakmogo`, `main`은 변경하지 않았다.
...
Pi의 고도화 DB는 새 `yakmogo-enhancement-mariadb-data` volume에 복원했다. 기존 host `yakmogo_enhancement`와 이전 8081 container는 rollback용으로 변경·삭제하지 않고 보존했다. 운영 8080, 운영 DB `yakmogo`와 runner는 변경하지 않았다.
...
10. 다음 실행도 `password: NO`였고 client가 인식하는 호환 환경변수 이름이 `MARIADB_PWD`가 아니라 `MYSQL_PWD`임을 확인했다. 명령 인자에 비밀번호를 노출하지 않으면서 해당 client process에만 `MYSQL_PWD`를 전달하도록 수정했다.
11. 전체 격리 통합 검증은 성공했지만 artifact가 폴더와 압축 파일에 같은 image를 두 번 담아 314 MB가 됐다. CI 검증에는 조립 폴더를 유지하고 다운로드 artifact에는 권한 보존 archive와 checksum만 포함하도록 중복을 제거했다.
12. GitHub artifact API는 비로그인 다운로드를 막았고 로컬 credential helper도 비대화식 token 제공을 거부했다. 사용자가 Actions artifact ZIP을 프로젝트 root에 내려받은 뒤 GitHub artifact digest와 내부 archive checksum을 모두 검증해 Pi로 전달했다.
13. 최초 root backup checksum은 절대경로를 기록해 안전한 사용자용 복사본에서 원본 root 경로를 읽으려 했다. 복원은 시작되지 않았고 새 DB는 빈 상태였다. 복사본 디렉터리에서 상대 파일명으로 checksum을 다시 생성한 뒤 복원을 성공시켰다. 프로젝트 `backup.sh`는 처음부터 상대 파일명 checksum을 사용한다.
14. MariaDB client password 변수 문제는 Pi에서도 같은 방식으로 재확인됐다. 최종 스크립트는 `MYSQL_PWD`를 해당 client process에만 전달하며 명령 인자나 출력에 비밀번호를 노출하지 않는다.
...
- GitHub ARM64 image build: 성공
- 빈 DB Flyway 설치: 성공
- SQL dump 백업과 새 volume 복원: 성공
- 복원 후 probe row 수 검증: 성공
- 고장 image readiness 실패와 직전 image rollback: 성공
...
- 최종 검증 commit: `e4e229c`
- [Backend CI run 29377588733](https://github.com/dlfjsld1/yakmogo/actions/runs/29377588733): 성공
- [Portable release run 29378240464](https://github.com/dlfjsld1/yakmogo/actions/runs/29378240464): 성공, 10분 56초
- artifact: `yakmogo-enhancement-e4e229cd5bca92fb7b47a8b7b3ab2e9825cc1948`
- artifact 크기: 157 MB
- artifact digest: `sha256:c4dab0f83c2abc248ada879b470671eb26df6144747562bc63e11614878cec12`

## Pi 실기 검증 결과

### DB 백업과 복원

- host DB: `yakmogo_enhancement`, 212,992 bytes
- root backup: `/var/backups/yakmogo-portable/yakmogo_enhancement-before-portable-20260715T010023Z.sql.gz`
- backup 크기: 1,856 bytes
- backup mode: `600`
- backup SHA-256: `5d8a07a5a3d4b5b4ebf18b1e99193c9a03213781b815b575a681a37f969d0527`
- 새 DB container: `yakmogo-enhancement-yakmogo-mariadb-1`, `mariadb:11.8`
- 새 volume: `yakmogo-enhancement-mariadb-data`
- DB port: host에 publish하지 않음
- host와 container의 5개 업무 table exact row count: 모두 0으로 일치
- Flyway version 1·2: 양쪽 모두 success
- user·group·intake 외래키 고아 row: 모두 0

### artifact와 설치

- GitHub artifact ZIP SHA-256: `c4dab0f83c2abc248ada879b470671eb26df6144747562bc63e11614878cec12`
- portable archive: `yakmogo-0.0.7-rc.9.1-portable.tar.gz`
- portable SHA-256: `e2a6e63c10ae4e7f16a4474e3ae1bd302703c5972560c7e21b1948d4e38e9722`
- ARM64 image SHA-256: `930e10a999f9a60a24ca06656336e27b1049f745647286f2e1aef2da4345e969`
- Pi 설치 경로: `/home/pi/yakmogo-portable`
- `.env`: owner `pi:pi`, mode `600`
- 앱 image: `yakmogo:0.0.7-rc.9.1`

### 18081 shadow와 8081 cutover

- 18081 `/`: 200
- 예상 JavaScript bundle: 200
- 보호 API: 401
- React 관리자 인증 화면 렌더링, browser console error 0
- Telegram bot·scheduler: `false`
- datasource: Docker DNS `yakmogo-mariadb:3306/yakmogo_enhancement`
- 8081 cutover 중단 시간: 38초
- 최종 8081 `/`: 200, JavaScript: 200, 보호 API: 401
- 이전 18081: 미응답으로 전환 완료
- 이전 `yakmogo-enhancement` container: `exited` 상태로 보존
- 운영 8080 PID `1794958`, HTTP 200 불변
- 기존 6개 비Yakmogo container ID 불변

### 실제 update rollback

정상 image를 테스트 tag로 복제하고 manifest의 JavaScript 경로만 존재하지 않는 값으로 바꿨다. `update.sh`는 readiness 실패를 감지해 exit 1을 반환하고 이전 `.env`, manifest와 `yakmogo:0.0.7-rc.9.1` image를 복원했다.

- rollback 시험: 성공
- 최종 8081 HTTP·JS·401: 성공
- update 전 DB backup: 1,839 bytes, mode 600
- 테스트 tar·directory·image tag: 삭제 확인
- 실제 DB volume과 MariaDB container: 재생성하지 않음
...
- GitHub-hosted runner가 일부 action의 Node.js 20 deprecation 경고를 출력한다. 현재는 Node.js 24로 강제 실행되어 성공했으며 action major version 갱신은 후속 CI 유지보수 후보로 남긴다.
- 이전 host `yakmogo_enhancement`와 이전 8081 container는 즉시 rollback을 위해 아직 보존 중이다. 제거 시점은 Goal 9의 보관·복원 정책과 함께 결정한다.

## 후속 Goal에 넘길 내용

검증된 feature를 `enhancement`에 병합한 뒤 Goal 9의 정기 백업, 보관 기간, 복원 리허설과 8080·8081 관측성으로 진행한다. 이전 host DB와 이전 8081 container 삭제는 Goal 9에서 별도 승인 대상으로 다룬다.
