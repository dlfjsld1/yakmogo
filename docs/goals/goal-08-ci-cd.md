...
**승인 지점 2 승인 후 활성화 준비 중.** 통합 JAR·manifest 생성, ARM64 self-hosted deploy job, 고정 경로 root helper, 자동 rollback 구현과 로컬 검증을 마쳤다. Raspberry Pi 설치와 실제 8081 배포·rollback 결과는 이 문서의 완료 결과에 이어 기록한다.
