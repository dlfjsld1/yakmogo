...
**완료, `enhancement` 병합 승인 대기.** `feature/backend-test-suite`에서 구현·로컬 테스트·실제 MariaDB CI·8081 비파괴 인수 테스트·문서화를 완료했다. 사용자 승인 전에는 `enhancement`에 병합하지 않는다.
...
- 커밋: `bd919b4` (`feat: harden backend API state transitions`)
- GitHub Actions run: `29309025944`
- `clean test bootJar`: 성공
- MariaDB 11.8 service: 성공
- MariaDB 전용 테스트 `skipped=0` 확인: 성공
- 실제 검증: enum, `BIT(1)`, TIME, FK 실패, 조건부 `PENDING → TAKEN` update

## 8081 배포 및 비파괴 인수 테스트

CI 성공 뒤 Goal 5 JAR을 8081에만 배포했다. 기존 JAR은 시도별 `backup-goal5-*` 디렉터리에 보존했다. 최종 성공 백업은 `/home/pi/myprojects/yakmogo-enhancement/backup-goal5-20260714-145716`이다.

| 요청 | 예상·실제 | 데이터 변경 |
|---|---|---|
| 인증 없이 `GET /api/v1/users` | 401 `UNAUTHORIZED` | 없음 |
| 관리자 `GET /api/v1/users` | 200 `[]` | 없음 |
| 미존재 사용자 조회 | 404 `NOT_FOUND` | 없음 |
| 음수 사용자 ID 조회 | 400 `VALIDATION_FAILED` | 없음 |
| 빈 이름 사용자 등록 | 400 `VALIDATION_FAILED` | validation에서 차단 |
| 미존재 복용 완료 | 404 `NOT_FOUND` | 없음 |

검증 결과:

| 항목 | 결과 |
|---|---|
| 배포 JAR SHA-256 | `c4b961b2b8ed0b5564555f428fd8fa28c48fba215b2c1278206518885e47ef6e` |
| 8081 | active, HTTP 200 |
| profile | `enhancement` |
| Flyway | version 1, history 1건, validate 성공 |
| 고도화 row count 전후 | `0 / 0 / 0 / 0` 동일 |
| Telegram·scheduler | enhancement profile의 false 유지 |
| 실제 Telegram 메시지 | 발송하지 않음 |
| 운영 8080 | active, HTTP 200, 재시작하지 않음 |
| 운영 row count | `1 / 1 / 2 / 65` 동일 |
| 임시 JAR·header·body·스크립트 | 삭제 확인 |

환경변수, systemd, DB schema는 변경하지 않았다. 운영 `main`, 8080, `yakmogo`에는 쓰기를 수행하지 않았다.

## 8081 검증 시행착오

첫 sudo 사전검사는 credential cache 만료로 JAR 교체 전에 중단됐다. PowerShell 문자열 pipeline은 native stdin 인코딩 때문에 같은 비밀번호가 잘못 전달됐다. 원격 `printf`로 바이트 전달을 고정한 뒤 해결했다.

새 JAR 기동 후에는 Linux `/tmp`의 보호 정책 때문에 root curl이 pi가 만든 임시 파일을 열지 못했다. header와 body를 root가 `/run`에서 처음 생성하도록 수정했다. 두 실패 모두 자동 rollback이 기존 JAR을 복원했고 8081·8080 HTTP 200과 기존 checksum을 확인한 뒤 재시도했다. 최종 시도는 전체 인수 테스트를 통과했다.
...
- 핵심 API 성공·400·401·403·404 테스트: 완료
- REST·Telegram 공통 상태 변경: 완료
- 중복·잘못된 전이·타 사용자 변경 차단: 완료
- DAILY·WEEKLY·INTERVAL 계산: 완료
- H2·실제 MariaDB 검증: 완료
- 기존 React payload·성공 응답 호환: 완료
- 로컬·feature CI·8081 비파괴 검증: 완료
- 한국어 Goal·트러블슈팅 문서: 완료
- `enhancement` 병합: 사용자 승인 대기
